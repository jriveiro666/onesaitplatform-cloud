...
public class JsonUtils {
	...
	private String path_data = "datos";
	...
	private final static String INTEGER_VAR = "integer";
	...
	private final static String FORMAT_VAR = "format";
	...
	public JsonNode generateJson(String ontology, String user) throws IOException {
		...
		path_data = refJsonSchema(ontologySchema);
		final String pathToProperties = (!ontologySchema.path(path_data).isMissingNode() ? path_data : PATH_PROPERTIES);

		final Iterator<String> fields = !ontologySchema.path(path_data).isMissingNode()
				...
				if (fieldNode.isArray()) {
					...
					for (int j = 0; j < size; j++) {
						...
						if (type.equals(STRING_VAR)) {
							...
							final JsonNode object = createObject(fieldNode);
							return object;
