...
public class ExternalJsonApiProcessor implements ApiProcessor {
	...
	private Map<String, Object> proxyHttp(Map<String, Object> data) {
		...
		try {
			...
			data.put(ApiServiceInterface.REASON,
	...
	private Map<String, Object> postProcess(Map<String, Object> data) {
		...
		if (apiManagerServiceConfig.postProcess(api) && method.equalsIgnoreCase("get")) {
            ...
            postProcess = postProcess.replace(ApiServiceInterface.CONTEXT_USER, user.getUserId());
			
			if (!StringUtils.isEmpty(postProcess)) {
				try {
					Object result = scriptEngine.invokeScript(postProcess, data.get(ApiServiceInterface.OUTPUT));
					data.put(ApiServiceInterface.OUTPUT, result);
	...
	private String getUrl(Swagger swagger, String pathInfo) {
		...
		if (swagger.getSchemes() == null || !swagger.getSchemes().stream().map(s -> s.name())
				.collect(Collectors.toList()).contains(ApiServiceInterface.HTTPS))
