...
public class ApiManagerService {
	...
	public String buildQuery(String queryDb, HashMap<String, String> queryParametersValues, User user) {
		for (final String param : queryParametersValues.keySet()) {
			queryDb = queryDb.replace("{$" + param + "}", queryParametersValues.get(param));
		}
		queryDb = queryDb.replace(ApiServiceInterface.CONTEXT_USER, user.getUserId());
		return queryDb;
	}
