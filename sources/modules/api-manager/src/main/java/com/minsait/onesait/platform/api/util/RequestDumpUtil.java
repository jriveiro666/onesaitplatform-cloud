...
public class RequestDumpUtil {
	...
	public String getAuthenticationHeader(HttpServletRequest request) {
		String toReturn = getValueFromRequest(ApiServiceInterface.AUTHENTICATION_HEADER, request);
		if (toReturn == null || toReturn.equals(""))
			toReturn = getValueFromRequest(ApiServiceInterface.AUTHENTICATION_HEADER_OLD, request);
		return toReturn;
	}
	...
	public void dumpRequest(StringBuilder sb, HttpServletRequest request) {
		...
		// sb.append(INDENT).append("[RequestedSessionId]
		// ").append(request.getRequestedSessionId()).append(LF);
