...
import com.minsait.onesait.platform.service.SolverServiceImpl;
...
public class SocketController {
	...
	public void response(@DestinationVariable("id") Long id, SimpMessageHeaderAccessor headerAccessor, InputMessage msg)
		...
		log.info("Query for: " + msg.getDs() + ",  params: filter: " + msg.getFilter() + ", project: " + msg.getProject() + ", group: " + msg.getGroup());
		
		String dataSolved = solverService.solveDatasource(msg);
		
		log.info("Query for: " + msg.getDs() + ",  params: filter: " + msg.getFilter() + ", project: " + msg.getProject() + ", group: " + msg.getGroup() + " executed in " + (System.currentTimeMillis() - startTime)/1000f + "(s)");
