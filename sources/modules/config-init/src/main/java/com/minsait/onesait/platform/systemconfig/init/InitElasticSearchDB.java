...
public class InitElasticSearchDB {
	...
	private static String ACCOUNTS_STR = "Accounts";
	private static String TYPE_TEXT = "            \"type\": \"text\",\n";
	private static String FIELDDATA_TRUE = "            \"fielddata\": true\n";
	...
	public void init_AccountsDataSet() {
		try {
	...
	public void createPostOperationsUser(User user, String collectionAuditName) {
		...
		if (ontologyService.getOntologyByIdentification(collectionAuditName, user.getUserId()) == null) {
			DataModel dataModel = datamodelRepository.findByName("AuditPlatform").get(0);
			final Ontology ontology = new Ontology();
