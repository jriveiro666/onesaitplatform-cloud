...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/", "/home", "/favicon.ico").permitAll()
				...
				.permitAll().regexMatchers("^/login/cas.*", "^/cas.*", "^/login*", "^/saml*").permitAll()
				...
				.authenticated().and().formLogin().successHandler(successHandler).permitAll().and().logout()
				.logoutSuccessHandler(logoutSuccessHandler).permitAll().and().sessionManagement().invalidSessionUrl("/")
				.maximumSessions(10).expiredUrl("/").maxSessionsPreventsLogin(false).sessionRegistry(sessionRegistry())
				.and().sessionFixation().none().and().exceptionHandling().accessDeniedHandler(accessDeniedHandler)
	...
	public KeyManager keyManager() {
		try {
			...
			ks.load(new FileInputStream(getClass().getClassLoader().getResource(samlJksUri).getFile()),
