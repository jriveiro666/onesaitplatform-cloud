...
public class ApiCreate {
	...
	public static ApiCreate createFromOPApi(Api api) {
		if (api.getApiType().equals(ApiType.EXTERNAL_FROM_JSON))
			...
			return ApiCreate.builder().version(api.getNumversion() + ".0").paths(new String[] { "/" })
					.contextPath(ApiCreate.getApiContextPath(api)).endpoint(api.getEndpoint())
					.description(api.getDescription()).name(api.getIdentification()).build();
	}
	...
	private static ApiCreate fromSwagger(Api api) {
		...
		return ApiCreate.builder().version(swagger.getInfo().getVersion())
				.paths(swagger.getPaths().keySet().toArray(new String[swagger.getPaths().size()])).endpoint(endpoint)
