...
public class MarketAssetHelper {
	...
	public void populateMarketAssetShowForm(Model model, String id) throws Exception {
		// Asset to show
		final MarketAsset marketAsset = marketAssetRepository.findById(id);
	...
	public String getApiDescription(String apiData) {
		...
		try {
			final String srcSwagger = resourcesService.getUrl(Module.apiManager, ServiceUrl.base)
					+ "swagger-ui.html?url=" + apiServices + "/management/swagger/v" + apis.get(0).getNumversion() + "/"
					+ apis.get(0).getIdentification() + "/swagger.json";
...
//				if (utils.isFileExtensionForbidden(marketAssetMultipart.getContent()))
//					throw new BinaryFileException("File type is not allowed");
	...
	public String getUrlWebProjectData(String webProjectData) {
		...
		final String webProjectUrl = rootWWW + webProject.getIdentification() + "/" + webProject.getMainFile();
