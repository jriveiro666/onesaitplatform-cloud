...
public class APIManagementController {
	...
	public ResponseEntity<?> deleteApiToken(
		...
		final UserToken token_obj = userTokenRepository.findByUserAndToken(userService.getUser(utils.getUserId()),
				token);
		if (token_obj == null)
	...
	public ResponseEntity<?> getApiList() {
		final List<com.minsait.onesait.platform.config.model.Api> apis = apiManagerService.loadAPISByFilter("", "", "",
				utils.getUserId());
		if (!apis.isEmpty()) {
			final ArrayList<ApiDTO> apisdto = new ArrayList<ApiDTO>();
	...
	public ResponseEntity<?> getApiByIdentification(
		...
		if (api != null) {
			if (apiManagerService.hasUserAccess(api.getId(), utils.getUserId())) {
				...
				return new ResponseEntity<String>("\" is not authorized \"", HttpStatus.UNAUTHORIZED);
			}
		} else {
			return new ResponseEntity<String>("{\"apis\" : \"\"}", HttpStatus.NOT_FOUND);
		}
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<?> deleteApi(@ApiParam(value = "Api id", required = true) @PathVariable("id") String apiId) {
		try {
	...
	public ResponseEntity<?> createApi(@ApiParam(value = "APIBody", required = true) @Valid @RequestBody ApiDTO apiBody,
		...
		if (api.getOntology() == null && api.getApiType().equals(ApiType.INTERNAL_ONTOLOGY))
			return new ResponseEntity<>("Missing Ontology", HttpStatus.BAD_REQUEST);
		if (api.getIdentification() == null || api.getIdentification().equals(""))
			return new ResponseEntity<>("Missing Api identification", HttpStatus.BAD_REQUEST);

		if (apiBody.getOperations() == null)
			return new ResponseEntity<>("Missing operations", HttpStatus.BAD_REQUEST);
	...
	private List<com.minsait.onesait.platform.config.model.ApiOperation> toAPIOperations(
		...
		for (ApiOperationDTO apiopdto : apiopsDTO) {
			...
			HashSet<ApiHeader> apiheader = new HashSet<ApiHeader>();
			...
			HashSet<ApiQueryParameter> apiqueryparam = new HashSet<ApiQueryParameter>();
		...
		if (api != null) {
			if (apiManagerService.hasUserAccess(api.getId(), utils.getUserId())) {
				if (!apiManagerService.validateState(api.getState(), state)) {
					return new ResponseEntity<String>("\"Forbidden change of state\"", HttpStatus.FORBIDDEN);
				}
				...
				return new ResponseEntity<String>("\" is not authorized\"", HttpStatus.FORBIDDEN);
			}
		} else {
