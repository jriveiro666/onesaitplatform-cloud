...
public class LayersRestImpl implements LayersRest {
	...
	final private static String PARAM = "param";
	...
	public ResponseEntity<?> getLayerData(HttpServletRequest request) {
		...
		try {
			...
			if (layer != null && (layer.getUser().equals(user)
				...
				if (query != null) {
					try {
						query = buildQuery(query, layer, request, userId);
						if (query == null) {
				...
				if (jsonArray != null) {
					for (Integer i = 0; i < jsonArray.length(); i++) {
						...
						if ((layer.getGeometryType().toLowerCase().equals(GeometryType.POINT.getName().toLowerCase()))
								|| layer.getGeometryType().toLowerCase().equals(RASTER)) {
						...
						} else if (layer.getGeometryType().toLowerCase()
								.equals(GeometryType.POLYGON.getName().toLowerCase())) {

							GeometryPolygon geometry = new GeometryPolygon();
							List<List<Double[]>> geoFinal = new ArrayList<List<Double[]>>();
							...
							feature.setGeometry(geometry);
								...
								.equals(GeometryType.MULTI_POLYGON.getName().toLowerCase())) {

							GeometryMultiPolygon geometry = new GeometryMultiPolygon();
							List<List<List<Double[]>>> geoFinal = new ArrayList<List<List<Double[]>>>();

							for (int y = 0; y < geo.length(); y++) {
								JSONArray geoAux = geo.getJSONArray(y);
								List<List<Double[]>> listAux = new ArrayList<List<Double[]>>();
								for (int z = 0; z < geoAux.length(); z++) {
									JSONArray g = geoAux.getJSONArray(z);
									List<Double[]> listAuxBis = new ArrayList<Double[]>();
									for (int x = 0; x < g.length(); x++) {
							...
							feature.setGeometry(geometry);
								...
								.equals(GeometryType.LINE_STRING.getName().toLowerCase())) {

							GeometryLinestring geometry = new GeometryLinestring();
							List<Double[]> geoFinal = new ArrayList<Double[]>();
				...
				Symbology symbology = buildSymbology(layer, layerIdentification);
	...
	private String buildQuery(String query, Layer layer, HttpServletRequest request, String userId) {
			...
			if (layer.getFilters() != null) {
				...
				for (int i = 0; i < filters.length(); i++) {
					...
					if (c.startsWith("#")) {
						...
						colorHex = String.format("#%02x%02x%02x", Integer.parseInt(split[0]),
