...
import com.minsait.onesait.platform.config.services.categorizationUser.CategorizationUserService;
...
public class CategorizationController {
	...
	public ResponseEntity<String> addAuthorization (@RequestParam("id") String id, @RequestParam("user") String userId,
		...
		if (!categorization.getUser().getUserId().equals(utils.getUserId()) || shareType.equals("OWNER")
				&& !utils.getRole().equals(Role.Type.ROLE_ADMINISTRATOR.toString())) {
			return new ResponseEntity<String>("{\"status\" : \"fail\"}", HttpStatus.BAD_REQUEST);
		}
		try {
	...
	public ResponseEntity<String> createCategorization (@RequestParam("name") String name, @RequestParam("json") String json) {
		try {
			if (categorizationRepository.findIdByIdentification(name) != null) {
				...
				throw new Exception("There is a Categorization Tree with the same Identification");
			}
			User user = userService.getUser(utils.getUserId());
	...
	public ResponseEntity<String> getCategorizationJson(@RequestParam(required = false) String id){
		String treeJson = new String();
		JSONArray jsonArray = new JSONArray();
		final List<CategorizationUser> activeCategorization = new ArrayList<CategorizationUser>();
		User user = userService.getUser(utils.getUserId());
		if (id != null) {
		...
		if (activeCategorization.size() == 1) {
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					JSONObject a_attrJson = new JSONObject(json.get("a_attr").toString());
					if (!a_attrJson.get("href").equals("#")) {
						if (getIdentification(a_attrJson.get("elementId").toString(), a_attrJson.get("elementType").toString()) != null) {
							json.put("text", getIdentification(a_attrJson.get("elementId").toString(), a_attrJson.get("elementType").toString()));
	...
	public ResponseEntity<String> getFlows(){
		try {
			...
			FlowDomain flow = flowRepository.findByUser_userId(utils.getUserId());
			if (flow != null) {
				identification = flow.getIdentification();
			}
			return new ResponseEntity<String>(identification, HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<String> getDataflows(){
		List<Pipeline> dataflows = new ArrayList<Pipeline>();
		User user = userService.getUser(utils.getUserId());
		try {
			JSONObject list = new JSONObject();			
