...
public class AppRestController {
	...
	public ResponseEntity<?> getRealms() {
		...
		if (utils.isAdministrator()) {
			...
			final List<Realm> realms = appService.getAppsByUser(utils.getUserId(), null).stream()
					.map(a -> new Realm(a, allRoles)).collect(Collectors.toList());
			return new ResponseEntity<>(realms, HttpStatus.OK);
		}
	}
	...
	public ResponseEntity<?> createAssociation(
		...
		if (!utils.isAdministrator()
				&& (((null == appDbChild.getUser()) || (!appDbChild.getUser().getUserId().equals(utils.getUserId())))
						|| ((null == appDbParent.getUser())
	...
	public ResponseEntity<?> getUser(
		...
		// final AppRole role = appDb.getAppRoles().stream().filter(r ->
		// r.getAppUsers().stream()
		// .map(u ->
		// u.getUser().getUserId()).collect(Collectors.toList()).contains(userId))
		// .findFirst().orElse(null);
		// if (role != null) {
		// final List<AppUser> users = role.getAppUsers().stream().filter(u ->
		// u.getUser().getUserId().equals(userId))
		// .collect(Collectors.toList());
		// if (users == null)
		// return new ResponseEntity<>(USER_STR + userId + "\" does not exist in realm
		// \"" + realmId + "\"",
		// HttpStatus.BAD_REQUEST);
		// else {
		// return new ResponseEntity<>(composeUser (users), HttpStatus.OK);
		// }
		// } else {
		// return new ResponseEntity<>(USER_STR + userId + NOT_EXIST,
		// HttpStatus.BAD_REQUEST);
		// }
	...
	public ResponseEntity<?> deleteUserRolAuthorization(
		...
		try {
			final AppRole role = appService.getByIdentification(realmId).getAppRoles().stream()
					...
					.findAny().orElse(null);
			if (role == null) {
				return new ResponseEntity<>("Authorization for user \"" + userId + "\" in Realm \"" + realmId
						+ "\" with role \"" + rolId + "\" not found.", HttpStatus.BAD_REQUEST);
			}
			appService.deleteUserAccess(role.getAppUsers().stream().filter(u -> u.getUser().getUserId().equals(userId))
					.filter(s -> s.getRole().getName().equals(rolId)).findFirst().get().getId());
			return new ResponseEntity<>(HttpStatus.OK);
		} catch (final AppServiceException e) {
			return new ResponseEntity<>(e, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	private RealmUser composeUser(List<AppUser> users) {
		final RealmUser user = RealmUser.builder().avatar(users.get(0).getUser().getAvatar())
				.extraFields(users.get(0).getUser().getExtraFields()).fullName(users.get(0).getUser().getFullName())
				.mail(users.get(0).getUser().getEmail()).role("").username(users.get(0).getUser().getUserId()).build();
		for (final AppUser appUser : users) {
			user.setRole(user.getRole() + "," + appUser.getRole().getName());
		}
		return user;
	}
