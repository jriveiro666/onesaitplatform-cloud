  ...
  //  vm.config=[{id:"filtro1",type:"numberfilter", field:"Helsinki.year",name:"year",op:">",typeAction:"filter",initialFilter:false,useLastValue:true,filterChaining:false,targetList:[{gadgetId:"livehtml_1550073936906",overwriteField:"Helsinki.year"},{gadgetId:"livehtml_1549895094697",overwriteField:"Helsinki.year"}],value:2000},
   ...
   return temp;
    ...
    vm.openEditGadgetIframe = function(ev) {
		...
		$.ajax({
          url: '/controlpanel/gadgets/hasUserPermission/'+vm.element.id,
          headers: {
                 ...
                 function DialogIframeEditGadgetController($scope, $mdDialog, element) {
            	  ...
            	  $mdDialog.show(
            	          $mdDialog.alert()
            	            ...
            	            .title('ALERT!')
            	            .textContent('You don\'t have permission to edit this Gadget.')
...
$templateCache.put('app/components/view/pageComponent/page.html','<div class=page-dashboard-container ng-style="{\'background-image\':\'url(\' + vm.page.background.filedata + \')\',\'background-color\': vm.page.background.color }"><synoptic ng-if="vm.synopticedit.showSynoptic && !vm.synopticedit.showEditor" style="position: absolute; z-index:1;" synoptic=vm.synoptic></synoptic><span ng-repeat="layer in vm.page.layers"><gridster ng-style="vm.dashboardheader.enable && {\'height\': \'calc(100% - \'+{{vm.dashboardheader.height}}+\'px\'+\')\'} || {\'height\': \'100%\'}" ng-if="(vm.page.combinelayers || vm.page.selectedlayer == $index) " options=vm.gridoptions class=flex><element ng-style="{\'z-index\':$parent.$index*500+1}" ng-if=item.id id={{item.id}} idtemplate={{item.idtemplate}} iframe=vm.iframe element=item editmode=vm.editmode ng-repeat="item in layer.gridboard"></element></gridster></span></div>');
$templateCache.put('app/components/view/synopticComponent/synoptic.html','<div id=synopticbody></div>');
$templateCache.put('app/components/view/synopticEditorComponent/synopticEditor.html','<iframe id=synoptic_editor ng-style="vm.dashboardheader.enable && {\'height\': \'calc(100% - \'+{{vm.dashboardheader.height + 4}}+\'px\'+\')\',\'position\': \'absolute\',\'z-index\':vm.config.zindexEditor} || {\'height\': \'99.5%\',\'position\': \'absolute\',\'z-index\':vm.config.zindexEditor}" src=/controlpanel/static/svg/editor/svg-editor.html width=99.5% onload=initsvgImage();></iframe>');
