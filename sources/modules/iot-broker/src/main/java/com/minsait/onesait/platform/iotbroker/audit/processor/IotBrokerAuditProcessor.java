...
public class IotBrokerAuditProcessor {
	...
	public OPAuditError getErrorEvent(SSAPMessage<? extends SSAPBodyMessage> message, GatewayInfo info,
		...
		if (message != null && info != null) {
			...
			if (iotEvent != null) {
				...
				if (session != null) {
					...
					event = OPEventFactory.builder().build().createAuditEventError(messageOperation,
							Module.IOTBROKER, ex);
				}
			}
			...
			event = OPEventFactory.builder().build().createAuditEventError("Exception Detected", Module.IOTBROKER,
					ex);
		}
