...
public class GatewayNotifierDelegate implements GatewayNotifier {
	...
	// @Override
	// public SSAPMessage<SSAPBodyReturnMessage>
	// sendCommandSync(SSAPMessage<SSAPBodyCommandMessage> cmd) {
	//
	// final List<CompletableFuture<SSAPMessage<SSAPBodyReturnMessage>>> f =
	// commands.values().stream().map(
	// c -> CompletableFuture.supplyAsync(() -> c.apply(cmd)))
	// .collect(Collectors.toList());
	//
	// final CompletableFuture<Object> future =
	// CompletableFuture.anyOf((CompletableFuture<?>[]) f.toArray());
	//
	// try {
	// final SSAPMessage<SSAPBodyReturnMessage> result =
	// (SSAPMessage<SSAPBodyReturnMessage>) future.get(5, TimeUnit.SECONDS);
	// return result;
