...
public class RouterFlowManagerService {
	...
	@PostConstruct
	public void init() {
		if (NODERED_THREAD_POOL_QUEUE < 0) {
			noderedNotificatorExecutor = Executors.newFixedThreadPool(NODERED_THREAD_POOL);
		} else {
			final BlockingQueue q = new ArrayBlockingQueue(NODERED_THREAD_POOL_QUEUE);
			noderedNotificatorExecutor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, q);
		}

		if (KAFKA_THREAD_POOL_QUEUE < 0) {
			kafkaNotificatorExecutor = Executors.newFixedThreadPool(KAFKA_THREAD_POOL);
		} else {
			final BlockingQueue q = new ArrayBlockingQueue(KAFKA_THREAD_POOL_QUEUE);
			kafkaNotificatorExecutor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, q);
		}
		if (RULES_ENGINE_THREAD_POOL_QUEUE < 0) {
			rulesEngineNotificatorExecutor = Executors.newFixedThreadPool(RULES_ENGINE_THREAD_POOL);
		} else {
			final BlockingQueue q = new ArrayBlockingQueue(RULES_ENGINE_THREAD_POOL_QUEUE);
		...
		try {
			...
			if (e.getCause() != null) {
				...
				if (e.getCause() instanceof DBPersistenceException) {
					...
					if (dbex.getErrorsResult().size() > 0)
						output.setMessage("{ \"error\" : { \"message\" : \"" + dbex.getDetailedMessage()
								+ "\", \"errors\" : " + (new Gson()).toJson(dbex.getErrorsResult()) + " }}");
					else if (dbex.getCause() != null)
						output.setMessage(
								"{ \"error\" : { \"message\" : \"" + dbex.getDetailedMessage() + "\", \"errors\" : ["
	...
	public NotificationCompositeModel executeCrudOperations(NotificationCompositeModel compositeModel)
		...
		// NotificationCompositeModel compositeModel = (NotificationCompositeModel)
		...
		try {
			...
			// exchange.getIn().setHeader(ERROR_STR, "");
		...
		// if (compositeModel.getOperationResultModel().isStatus() == false) {
		// exchange.getIn().setHeader(ERROR_STR, "");
		// }
