...
public class KafkaService {
	...
	private static String KAFKA_DEFAULT_PORT = "9092";

	private AdminClient adminAcl;

	private void applySecurity(Properties config) {
		if (!KAFKA_DEFAULT_PORT.equals(kafkaPort)) {
	...
	public boolean createTopicForOntology(String name) {
		NewTopic t = new NewTopic(getTopicName(name), partitions, replication);
		try {
			log.info("Kafka configuration to be applied, partitions: " + partitions + " replication: " + replication);
	...
	public boolean createTopic(String name) {
		...
		try {
			...
			log.info("Cannot ensure topic creation for  '{}'", name);
			return false;
		}
	}
