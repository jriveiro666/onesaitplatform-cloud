...
public class JPAHAS256ConverterCustom implements AttributeConverter<String, String> {
	...
	public static String storedFlag = "abcdefghijklmnopqrstuv";// Not match with valid password, cannot be introduced by
	// form
	public static String SHA_PREFIX = "SHA256(";

	@Override
	public String convertToDatabaseColumn(String sensitive) {
		try {

			if (sensitive.startsWith(storedFlag)) {// The password is encoded, The entity was stored previously and
													// password didn't change
				return sensitive.substring(storedFlag.length());
