...
public class KuduQueryAsTextDBRepository implements QueryAsTextDBRepository {
	...
	private static final String insertKuduPrefix = "insert";
	private static final String updateKuduPrefix = "update";
	private static final String selectKuduPrefix = "select";
	private static final String deleteKuduPrefix = "delete";
	...
	public String queryNativeAsJson(String ontology, String query, int offset, int limit)
		...
		try {
			...
			if(queryLC.startsWith(updateKuduPrefix)) {
				return KuduBasicOpsDBRepository.updateNative(ontology, procQuery, false).toString();
			}
			else if(queryLC.startsWith(deleteKuduPrefix)){
			...
			throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.KUDU, e.getMessage() ), e.getMessage());
		}
	}
