...
public class KuduBasicOpsDBRepository implements BasicOpsDBRepository {
	...
	public ComplexWriteResult insertBulk(String ontology, String schema, List<String> instances, boolean order,
		...
		List<BulkWriteResult> result = new ArrayList<BulkWriteResult>();
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			case UNIXTIME_MICROS:
				prow.addTimestamp(key, new Timestamp(ISODateTimeFormat.dateTimeParser()
						.parseDateTime(((String) o).substring(1, o.length() - 1)).getMillis()));
				break;
			case BOOL:
		...
		for (String whereExp : whereList) {
			...
			if (fieldVal.length != 2) {
				...
				case STRING:
					...
					break;
				case UNIXTIME_MICROS:
					prow.addTimestamp(key, Timestamp
							.valueOf(((String) o).substring(1, o.length() - 1).replace("T", " ").replace("Z", "")));
					break;
				case BOOL:
					prow.addBoolean(key, Boolean.getBoolean(o));
					break;
