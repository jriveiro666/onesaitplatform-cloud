		...
		while(keys.hasNext()) {
		    ...
		    if (obj.get(key) instanceof JSONObject) {
				...
				if(JSONObject.NULL.equals(o)) {
					...
					if (!onlyPKey || (onlyPKey && schema.getColumn(key + HiveFieldType.LONGITUDE_FIELD).isKey())) {
						prow.addTimestamp(key, new Timestamp(ISODateTimeFormat.dateTimeParser().parseDateTime((String)o.get("$date")).getMillis()));
					}
					...
					if (!onlyPKey || (onlyPKey && schema.getColumn(CONTEXT_DATA_FIELD_DEVICE_TEMPLATE_CONNECTION).isKey())) {
						prow.addString(CONTEXT_DATA_FIELD_DEVICE_TEMPLATE_CONNECTION, (String) o.get(FIELD_DEVICE_TEMPLATE_CONNECTION));
					}
				...
				if (!onlyPKey || (onlyPKey && schema.getColumn(key).isKey())) {
					...
					switch(schema.getColumn(key).getType()) {
						...
						case UNIXTIME_MICROS:
							prow.addTimestamp(key, new Timestamp(ISODateTimeFormat.dateTimeParser().parseDateTime((String)o).getMillis()));
							break;
						case BOOL:
