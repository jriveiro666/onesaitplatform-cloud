...
public class ExternalApiRestOpsDBRepository implements BasicOpsDBRepository {
	...
	private final static String NOT_SUPPORTED = "Operation not supported for Api Rest Ontologies";
	...
	public String querySQLAsJson(String ontology, String query) throws DBPersistenceException {
		...
		if (operationName == null) {
			...
			if (operation == null) {
				...
				// throw new NotSupportedOperationException("The specified operation '" +
		...
		if (response.getResponse() == 200) {
			...
			if (!analyzedQuery.isSelectAll() && selectedFfields.size() > 0) {
			...
			throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
					"Unable to run query in REST API Datasource. Error " + response.getResponse() + "."
							+ response.getBody());
			// throw new DBPersistenceException("Unable to run query in REST API Datasource.
			// Error "
			// + response.getResponse() + "." + response.getBody());
		}
	}
	...
	public String findAllAsJson(String ontology) throws DBPersistenceException {
		return querySQLAsJson(ontology, "SELECT * FROM " + ontology);
	}

	@Override
	public String findAllAsJson(String ontology, int limit) throws DBPersistenceException {
		return querySQLAsJson(ontology, "SELECT * FROM " + ontology + " LIMIT " + limit);
	}
	...
	public List<String> findAll(String ontology, int limit) throws DBPersistenceException {
		String json = querySQLAsJson(ontology, "SELECT * FROM " + ontology);
		if (limit > 0)
			json += " LIMIT " + limit;
			...
			if (response.getResponse() == 200) {
				...
				throw new DBPersistenceException(new ErrorResult(ErrorResult.PersistenceType.API, response.getBody()),
						"Unable to delete data in REST API Datasource. Error " + response.getResponse() + "."
								+ response.getBody());
				// throw new DBPersistenceException("Unable to insert data in REST API
				// Datasource. Error "
				// + response.getResponse() + "." + response.getBody());
			}
		} else {
	...
	private Map<String, String> getHeadersAndSecurity(OntologyRest ontologyRest) {
		...
		try {
			securityConf = mapper.readValue(ontologyRest.getSecurityId().getConfig(), mapType);
		...
		try {
			List<Map<String, String>> headersList = mapper.readValue(headersConfig, typeRef);
