...
public class SelectSyntaxAnalyzer {
	public QueryAnalysisResult selectStatementProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.ALL_FIELDS || token.getType() == TokenType.WORD) {
				// result.setPorjectionFields(Arrays.asList(token.getContent()));
				result.addFieldToProjection(token.getContent());
				tokens.remove(0);
				return fromStatementProcessing(result, tokens);
			}
			result.setErrorMessage("Expecting Field or * but received " + token.toString() + "  token.");
		} else {
	...
	private QueryAnalysisResult fromOperationStatementProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.OPERATION_SEPARATOR) {
				...
				result.setErrorMessage("Expecting OPERATION name but received " + token.toString() + "  token.");
				result.setState(false);
				return result;
			}
		}
		...
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.AS) {
				...
				if (tokens.size() > 0) {
					...
					result.setErrorMessage("Expecting alias name but received " + token.toString() + "  token.");
				} else {
	...
	private QueryAnalysisResult whereStatementProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			result.setErrorMessage("Expecting WHERE clause or END token but received " + token.toString() + "  token.");
			result.setState(false);
			return result;
		}
	...
	private QueryAnalysisResult whereFilterStatementProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.WORD) {
				...
				result.setErrorMessage("Where clause parentheses are not closd properly.");
			} else if (token.getType() == TokenType.LIMIT || token.getType() == TokenType.SKIP) {
				if (result.checkParenthesesCount()) {
					return skipOrLimitProcessing(result, tokens);
				}
		...
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.FOP) {
				...
				if (tokens.size() > 0) {
					...
					result.setErrorMessage(
							"Expecting Value after field comparator but received " + token.toString() + "  token.");
				} else {
					result.setErrorMessage("Expecting Value after field comparator but no tokens were received.");
				}
			} else {
				result.setErrorMessage(
						"Expecting Field comparator '=', '!=', '>',... but received " + token.toString() + "  token.");
			}
		...
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.LIMIT) {
				...
				if (token.getType() == TokenType.WORD) {
					...
					result.setErrorMessage("Expexting LIMIT value but " + token.toString() + " token was received");
				}
				...
				if (token.getType() == TokenType.WORD) {
					...
					result.setErrorMessage("Expexting SKIP value but " + token.toString() + " token was received");
				}
			} else if (token.getType() == TokenType.LIMIT) {
				result.setState(true);
				return result;
			} else {
				result.setErrorMessage(
						"Expexting LIMIT or SKIP clause but " + token.toString() + " token was received");
			}
