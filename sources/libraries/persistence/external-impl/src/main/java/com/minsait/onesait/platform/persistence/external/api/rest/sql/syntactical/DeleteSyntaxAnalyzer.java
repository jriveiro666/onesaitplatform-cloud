...
@Component("DeleteSyntaxAnalyzer")
@Lazy
public class DeleteSyntaxAnalyzer {
	public QueryAnalysisResult deleteStatementProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.FROM) {
				...
				result.setErrorMessage("Expecting INTO but received " + token.toString() + "  token.");
			}
		} else {
	...
	private QueryAnalysisResult closeParenthesesOrOpProcessing(QueryAnalysisResult result, List<Token> tokens) {
		if (tokens.size() > 0) {
			...
			if (token.getType() == TokenType.RIGHT_PARENTHESES) {
				...
				result.setErrorMessage(
						"Expecting ')', 'AND' or 'OR' clause but received " + token.getType().toString() + "  token.");
			}
		} else {
