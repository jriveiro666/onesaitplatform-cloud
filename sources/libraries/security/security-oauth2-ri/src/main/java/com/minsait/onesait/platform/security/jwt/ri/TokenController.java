...
public class TokenController {
	...
	public ResponseToken renewToken(@RequestBody String id) {
		try {
			...
			OAuth2Authentication authentication = customTokenService.loadAuthentication(id);
			OAuth2AccessToken token = customTokenService.getAccessToken((OAuth2Authentication) authentication);
			...
			OAuth2AccessToken tokenRefreshed = customTokenService.refreshAccessToken(refreshToken.getValue(),
					tokenRequest);
	...
	public ResponseToken info(@RequestBody String tokenId) {
		try {
			...
			OAuth2Authentication authentication = customTokenService.loadAuthentication(tokenId);
	...
	@ResponseBody
	public Map<String, ?> revokeAccesToken(@RequestHeader(value="Authorization") String authorization, @RequestParam("token") String value) {
		...
		try {
			//createNewToken ();
			String[] tokens = sofia2TokenUtil.extractAndDecodeHeader(authorization);
			assert tokens.length == 2;
			
			appId = tokens[0];
			appSecret = tokens[1];
			
			ClientDetails clientId = clientDetailsService.loadClientByClientId(appId);
			...
			OAuth2Authentication authentication = customTokenService.loadAuthentication(value);
			OAuth2AccessToken token = customTokenService.getAccessToken((OAuth2Authentication) authentication);
			
			String appTokenId = (String) token.getAdditionalInformation().get("clientId");
	...
	@ResponseBody
	public Map<String, ?> accessInfo(@RequestHeader(value="Authorization") String authorization, @RequestParam("token") String value) {
		Map<String, Object> response = new HashMap<String, Object>();
