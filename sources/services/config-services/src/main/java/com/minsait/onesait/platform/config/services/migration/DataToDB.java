...
public class DataToDB {
	...
	public LoadEntityResult getEntitiesFromData(MigrationConfiguration config, DataFromDB data, EntityManager em) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException, ClassNotFoundException{
		...
		for(int i = 0; i < config.size(); i++) {
			...
			if (!entities.exist(clazz, id)) {
				...
				if (dbInstance != null) {
					allErrors.addError(new MigrationError(new Instance(clazz,id), null, MigrationError.ErrorType.WARN, "The entity is already in the database, nothing was done"));
				} else {
		...
		LoadEntityResult loadEntityResult = new LoadEntityResult(entities.getAll(), allErrors);
		return loadEntityResult;
	}

	private Object getEntityFromData(Class<?> clazz, Serializable id, DataFromDB data, EntityManager em, MigrationErrors errors, EntityCache visited, MigrationConfiguration config) throws InstantiationException, IllegalAccessException, IllegalArgumentException, ClassNotFoundException {
		...
		if (entities.exist(clazz, id)) {
			...
			for (String fieldName : orderedFields) {				
				...
				if(!Modifier.isFinal(field.getModifiers())) {
					...
					Object value = getCorrectValue(instanceData.get(fieldName), field.getType(), field.getGenericType(), data, em, errors, newVisited, config, rootInstance);
					if (value instanceof MigrationError) {
						...
						MigrationError migrationError = new MigrationError(instanceThatNeedsIt, errorReturned.getNeededInstance(), MigrationError.ErrorType.ERROR, "The instance depends on a non available entity");
	...
	private Object getCorrectValue(Object value, @SuppressWarnings("rawtypes") Class fieldType, Type type, DataFromDB data, EntityManager em, MigrationErrors errors, EntityCache visited, MigrationConfiguration config, Instance rootInstance) throws IllegalArgumentException, IllegalAccessException, InstantiationException, ClassNotFoundException  {
		...
		if (value != null) {
				...
				if (MigrationUtils.isManagedEntity(em, fieldType)) {
					...
					if (entity == null) {
						if (data.isDataStored(fieldType, sValue)) {
							//TODO test the possible collisions of get here entity in addition to do it in the calling method.
							//TODO analyze the impact of return a LoadResult with errors and list of entities. This would allow to add 
							//     dependent entities.
							Object entityFromData = getEntityFromData(fieldType, sValue, data, em, errors, visited, config);
							
							//this for cases in which not all data is in config, but there are dependencies that are needed.
							//TODO implement a different way to avoid to pass the iterator
							if (!config.contains(fieldType, sValue) && !entities.exist(fieldType, sValue) && !entitiesWithErrors.exist(fieldType, sValue)) {
								
								Instance instance = new Instance(fieldType, sValue);
								config.add(instance);
							
								MigrationError error = new MigrationError(rootInstance,instance,MigrationError.ErrorType.WARN, "Entity not configured added because other entity needs it");
								errors.addError(error);
							}
							return entityFromData;
						} else {
							Instance instance = new Instance(fieldType, sValue);
							MigrationError error = new MigrationError(null,instance,MigrationError.ErrorType.ERROR, "Mandatory entity");
							errors.addError(error);
							return error;
						}
					}
			...
			if (Date.class.isAssignableFrom(fieldType)) {				
				...
				ZonedDateTime parsedValue = ZonedDateTime.parse((String) value);
				return parsedValue;
			} else if (byte[].class.equals(fieldType)) {
				String string64 = (String) value;
				byte[] bytes = Base64.getDecoder().decode(string64.getBytes(StandardCharsets.ISO_8859_1));
				return bytes;
			} else if (Collection.class.isAssignableFrom(fieldType)) {					
				Collection<?> collection = (Collection<?>) value;
				@SuppressWarnings("unchecked")
				Collection<Object> newCollection = (Collection<Object>) MigrationUtils.createCollection(fieldType);
				for (Object obj : collection) {
					...
					Object correctValue = getCorrectValue(obj, objClass, null, data, em, errors, visited, config, rootInstance);
				...
				//At this point, if the data come from a json, arrays are deserialized as Arraylists.
				//If data come from java objects directly they are Arrays. (Data did not pass the serialization/deserialization process)
