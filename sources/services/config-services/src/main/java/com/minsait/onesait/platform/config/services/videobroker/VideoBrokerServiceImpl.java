...
public class VideoBrokerServiceImpl implements VideoBrokerService {
	...
	public final static String VIDEO_RESULT_DATA_MODEL = "VideoResult";
	...
	public boolean hasUserAccess(String id, String userid) {
		...
		final User user = userService.getUser(userid);
		if (vc != null && vc.getUser().equals(user) || user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.name())) {
				return true;
		}
		return false;
	}
