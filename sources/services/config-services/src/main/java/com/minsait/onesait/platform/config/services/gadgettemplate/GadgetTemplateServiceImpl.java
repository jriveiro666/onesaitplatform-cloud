...
public class GadgetTemplateServiceImpl implements GadgetTemplateService {
	...
	public List<GadgetTemplate> findGadgetTemplateWithIdentificationAndDescription(String identification,
		...
		if (user.getRole().getId().equals(ADMINISTRATOR)) {gadgetTemplates = this.gadgetTemplateRepository.findAll();}
		else {gadgetTemplates = this.gadgetTemplateRepository.findGadgetTemplateByUserAndIsPublicTrue(user.getUserId());}
