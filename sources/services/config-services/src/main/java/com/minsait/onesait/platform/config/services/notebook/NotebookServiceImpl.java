...
public class NotebookServiceImpl implements NotebookService {
	...
	public Notebook importNotebookFromJupyter(String name, String data, String userId)
		...
		try {
			...
			throw new NotebookServiceException(Error.INVALID_FORMAT_NOTEBOOK);
		}
		return importNotebook(name, formatedData, userId);
	...
	public Notebook cloneNotebook(String name, String idzep, String userId)
		...
		if (hasUserPermissionInNotebook(nt, userId)) {
			...
			throw new NotebookServiceException(Error.PERMISSION_DENIED);
		}
	}
	...
	public ResponseEntity<String> restartInterpreter(String interpreterName, String body, User user)
		...
		try {
			...
			if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || 
				...
				log.error("User " + user.getUserId() + " has not permission to restart" + interpreterName + ".");
				JSONObject responseBody = new JSONObject().put("error","User " + user.getUserId() + " has not permission to restart interpreter "+ interpreterName + ". Check interpreter configuration.");
	...
	private String getInterpreterId(String interpreterName) 
		...
		try {
			...
			log.error("Unable to get interpreter " + interpreterName + " id to restart");
			throw new NotebookServiceException("Unable to get interpreter " + interpreterName + " id");	
		}
	...
	private boolean isSharedInterpreterConfiguration(String interpreterName, String interpreterId) 
		...
		try {
			...
			log.error("Unable to get interpreter " + interpreterName + " configuration to restart");
			throw new NotebookServiceException("Unable to get interpreter " + interpreterName + " configuration");
		}	
		...
		for (Map.Entry<String, String> entry : notebookInterpreters.entrySet()) {
			...
			try {
				if (user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.toString()) || 
					...
					log.error("User " + user.getUserId() + " has not permission to restart interpreter " + entry.getKey() + ". Check interpreter configuration.");
					responseBody.put(key, "ERROR: User has not permission to restart interpreter.");
				}			
			} catch (Exception e) {
	...
	private String getDefaultInterpreter() throws ClientProtocolException, URISyntaxException, IOException {
		ResponseEntity<String> responseEntity;
		...
		try {
			...
			log.error("Unable to get default interpreter");
			throw new NotebookServiceException("Unable to get default interpreter");	
		}
	...
	private Map<String, String> getAllInterpretersNameAndId() throws ClientProtocolException, URISyntaxException, IOException {
	...
	private Map<String, String> getNotebookInterpreters(String notebookId) throws ClientProtocolException, URISyntaxException, IOException {
		ResponseEntity<String> responseEntity;
