...
public class ApiManagerServiceImpl implements ApiManagerService {
	...
	public void updateApi(Api api, String deprecateApis, String operationsObject, String authenticationObject) {
		...
		if (api.getApilimit() != null) {
			if (api.getApilimit() <= 0) {
				api.setApilimit(1);
			} else {
				apimemory.setApilimit(api.getApilimit());
			}
		} else {
	...
	public String updateApiRest(Api apinew, Api apimemory, List<ApiOperation> operations,
		...
		if (apinew.getApilimit() != null) {
			if (apinew.getApilimit() <= 0) {
				apimemory.setApilimit(1);
			} else {
				apimemory.setApilimit(apinew.getApilimit());
			}
		} else {
			apimemory.setApilimit(null);
		}
