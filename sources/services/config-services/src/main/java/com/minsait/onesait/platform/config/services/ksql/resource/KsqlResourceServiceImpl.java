...
public class KsqlResourceServiceImpl implements KsqlResourceService, KafkaTopicNotificationService {
	...
	private Pattern creation = Pattern.compile(
			"(?i)\\s*CREATE\\s+(TABLE|STREAM)\\s+(\\w+)\\s+(\\(((?!\\)\\s+WITH\\s+\\().)+\\))\\s*(WITH\\s+\\((\\w+\\s*\\=\\s*'[\\w]+')?(\\s*,\\s*(\\w+\\s*\\=\\s*'[\\w]+'))*\\))?.*;");
	private Pattern creationAs = Pattern.compile(
	...
	public void updateKsqlResource(KsqlResourceForUpdate ksqlresourceForUpdate) throws KsqlExecutionException {
		...
		KsqlResource ksqlResource = ksqlresourceForUpdate.getChangedKsqlReousrce();
		...
		if (ksqlresourceForUpdate.isStatemenChanged()) {
			...
			if (!ksqlresourceForUpdate.getCurrentIdentification().equals(ksqlResource.getIdentification())) {
				this.validateKsqlResource(ksqlResource);
			}
			try {
	...
	public List<String> parseStatementTextAndGetDependencies(KsqlResource ksqlResource) {
		...
		if (matcher.matches()) {
			...
			String withClause = matcher.group(withGroup);

			if (hasWithClause && withClause != null) {
				String[] listOfPorperties = withClause.trim().replaceAll("(?i)^WITH\\s*\\(", "")
						.replaceAll("\\s*\\).*", "").split(",");
	...
	public List<String> getKafkaTopicNotification(String ontology) {
		return ksqlResourceRepository.findByOntology_IdentificationAndResourceType(ontology, FlowResourceType.ORIGIN);
	}
