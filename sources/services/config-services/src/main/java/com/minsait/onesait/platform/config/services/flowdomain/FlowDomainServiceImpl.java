			...
			for (final FlowNode node : nodes) {
				if (node.getFlowNodeType() == Type.API_REST) {
					try {
						...
						log.error("Unaable to remove the defined API {} while deploying.", node.getIdentification());
						throw new FlowDomainServiceException(
								"Unaable to remove the defined API " + node.getIdentification() + " while deploying.");
					}
				}
