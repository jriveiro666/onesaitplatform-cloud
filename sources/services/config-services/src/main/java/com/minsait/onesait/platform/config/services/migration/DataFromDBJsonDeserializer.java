...
public class DataFromDBJsonDeserializer extends StdDeserializer<DataFromDB>{
	...
	@Override
	public DataFromDB deserialize(JsonParser p, DeserializationContext ctxt)
		...
		if (jsonClasses.isArray()) {
			...
			try {
				...
				throw new RuntimeException("Error obtaining classes", e);
			}
		} else {
			throw new RuntimeException("Error processing json: it should be an array");
		}
		
		return data;
	}

	private void processInstances(ObjectMapper mapper, MapType mapType, DataFromDB data, Class<?> clazz, JsonNode instances) throws JsonProcessingException, IOException {
		...
		if (instances.isArray()) {
			...
			throw new RuntimeException("Error processing json: instances field should be an array");
		}
