...
public class OPResourceServiceImpl implements OPResourceService {
	...
	public void createUpdateAuthorization(ProjectResourceAccess pRA) {
		...
		if (pRADB != null) {
			...
			// pRA.getResource().getProjectResourceAccesses().remove(pRADB);
		}
	...
	@Override
	@Transactional
	public void removeAuthorization(String id, String projectId, String userId) {
		...
		// pra.getResource().getProjectResourceAccesses().removeIf(p -> p.equals(pra));
	...
	public void insertAuthorizations(Set<ProjectResourceAccess> accesses) {
		...
		repeated.forEach(pra -> {
			pra.getProject().getProjectResourceAccesses().remove(pra);
			// pra.getResource().getProjectResourceAccesses().remove(pra);
		});
