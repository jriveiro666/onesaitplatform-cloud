...
public class CacheBusinessServiceImpl implements CacheBusinessService{
    ...
    @Override
    @Transactional
    public <K,V> Cache create(Cache cacheData) throws CacheBusinessServiceException{        
        Cache cache = cacheService.createMap(cacheData);      
        if (cache != null) {
            
                                  
            if (existCacheObject(cache)) {
                throw new CacheBusinessServiceException(CacheBusinessServiceException.Error.NAME_OF_MAP_ALREADY_USED, "Error creating map");
            }
            ...
            default:
                throw new CacheBusinessServiceException(CacheBusinessServiceException.Error.UNSUPPORTED_TYPE, "The cache type is not supported");
            }
            
            
        } else {
            throw new CacheBusinessServiceException(CacheBusinessServiceException.Error.CACHE_WAS_NOT_CREATED, "Error creating map");
        }
    }
    ...
    private boolean existCacheObject(Cache cache) {
      //check if the map already exists to avoid random access to previously created maps
    ...
    @Override
    public<K,V> void putIntoMap(String identification, K key, V value, User user) throws CacheBusinessServiceException {
