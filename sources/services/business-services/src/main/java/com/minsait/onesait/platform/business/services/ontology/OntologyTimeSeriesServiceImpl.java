...
public class OntologyTimeSeriesServiceImpl implements OntologyTimeSeriesService {
	...
	@Override
	@Transactional
	public ResponseEntity<?> createOntologyTimeSeries(OntologyTimeSeriesDTO ontologyTimeSeriesDTO,
			HttpServletRequest request)
			throws OntologyServiceException, OntologyDataJsonProblemException, OntologyBusinessServiceException {
		...
		ontologyBusinessService.createOntology(ontology, ontology.getUser().getUserId(), config);
		...
		for (OntologyTimeSeriesProperty oTSP : ontologyTimeSeriesDTO.getTimeSeriesProperties()) {
			oTSP.setOntologyTimeSeries(oTS);
			ontologyTimeSeriesPropertyRepository.save(oTSP);
		}
		for (OntologyTimeSeriesWindow oTSW : ontologyTimeSeriesDTO.getTimeSeriesWindow()) {
			oTSW.setOntologyTimeSeries(oTS);
			ontologyTimeSeriesWindowRepository.save(oTSW);
		}

		if (ontologyTimeSeriesDTO.isStats()) {
			Ontology stats = new Ontology();
			stats.setJsonSchema("{\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"title\":\""
					+ ontologyTimeSeriesDTO.getIdentification() + "_stats" + "\",\"type\":\"object\",\"required\":[\""
					+ "Stats" + "\"],\"properties\":{\"" + ontologyTimeSeriesDTO.getIdentification() + "_stats"
			...
			stats.setIdentification(ontologyTimeSeriesDTO.getIdentification() + "_stats");
			final OntologyConfiguration config2 = new OntologyConfiguration(request);

			ontologyBusinessService.createOntology(stats, stats.getUser().getUserId(), config2);
		}
	...
	public ResponseEntity<?> updateOntologyTimeSeries(OntologyTimeSeriesDTO ontologyTimeSeriesDTO, String sessionUserId,
		...
		OntologyTimeSeries oTS = ontologyTimeSeriesRepository.findById(ontologyTimeSeriesDTO.getId());
		...
		// Check and add new
		for (OntologyTimeSeriesProperty oTSP : ontologyTimeSeriesDTO.getTimeSeriesProperties()) {
			for (OntologyTimeSeriesProperty oTSPDB : ontologyTSPDb) {
		...
		// Check and add new
		for (OntologyTimeSeriesWindow oTSW : ontologyTimeSeriesDTO.getTimeSeriesWindow()) {
			for (OntologyTimeSeriesWindow oTSWDB : ontologyTSWDb) {
		...
		/*
		 * for (OntologyTimeSeriesWindow oTSW : ontologyTSWDb) { if
		 * (!ontologyTimeSeriesDTO.getTimeSeriesWindow().contains(oTSW)) {
		 * ontologyTimeSeriesWindowRepository.deleteById(oTSW.getId()); } }
		 */
