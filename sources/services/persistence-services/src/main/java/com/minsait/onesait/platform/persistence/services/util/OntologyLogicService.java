...
public class OntologyLogicService {
	...
	public Map<String, String> getAdditionalDBConfig(Ontology ontology){
		...
		try {
			...
			throw new OntologyLogicServiceException("Problems getting internal DB config of the ontology." + e.getMessage(), e);
		}
	}
		...
		for (Map.Entry<String, String> entry : prevConfig.entrySet()) {
			...
			if(configValue == null) {
				if(configToConfigMap.containsKey(configKey) && configToConfigMap.get(configKey) != null) {
					throw new OntologyLogicServiceException("Incompatible internal config DB: " + configKey + ", previous value null, next value " + configToConfigMap.get(configKey));
				}
			}
			else {
				if(!configToConfigMap.containsKey(configKey) || configToConfigMap.get(configKey) == null || !configToConfigMap.get(configKey).equals(configValue)) {
					throw new OntologyLogicServiceException("Incompatible internal config DB: " + configKey + ", previous value " + configValue + ", next value " + (!configToConfigMap.containsKey(configKey) || configToConfigMap.get(configKey)==null?"null":configToConfigMap.get(configKey)));
				}
			}
		}
	}
